 ...
 dateupCoEvent = data.events.filter( element =>(new Date(element.date) > dateCurrentDate))
 ...
 checkElemt.forEach((checkbox) => {
   checkbox.addEventListener("change", () => {
     ...
     const filtered = dateupCoEvent.filter((obj) => {
 ...
 function huntSearch() {
   ...
   const eventsFiltered = dateupCoEvent.filter(event => {
     const nameF = event.name.toLowerCase().trim().includes(busqueda);
     const descF = event.description.toLowerCase().trim().includes(busqueda);
     return nameF || descF;
