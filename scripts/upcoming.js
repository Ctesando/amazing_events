...
async function winData(){
  try{
    ...
    datosMyApi = await response.json()
     ...
     dateupCoEvent = findPastEvents(datosMyApi)
   
    await filterCategories(dateupCoEvent)
    showCategories(categorias) 
    huntSearch(dateupCoEvent)
    renderCards(dateupCoEvent)
    printCard(dateupCoEvent, "#row")
  } catch(error){
...
const botonSearch = document.getElementById("search")
 ...
 const resultados = dateupCoEvent.filter(evento => 
